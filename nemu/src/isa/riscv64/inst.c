...
#define immJ()  do { *imm = SEXT(((BITS(i, 31, 31) << 19) | (BITS(i, 19, 12) << 11) | \
                                  (BITS(i, 20, 20) << 10) | BITS(i, 30, 21)), 20) << 1; } while(0)
#define immB()  do { *imm = SEXT(((BITS(i, 31, 31) << 11) | (BITS(i, 7, 7) << 10) | \
                                  (BITS(i, 30, 25) << 4)  | BITS(i, 11, 8)), 12) << 1; } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(rd) = R(rd) = src1 - src2);
