...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, {R(rd) = s->pc + 4; s->dnpc = src1 + imm; 
                                                                 s->isa.inst.val == 0x00008067? ftrace_ret(): ftrace_call(); });
                                                                 // ret --> [[jalr x0, 0(x1)]]
  ...
  INSTPAT("000000? ????? ????? 001 ????? 00110 11", slliw  , I, R(rd) = SEXT(BITS(src1, 31, 0) << BITS(imm, 5, 0), 32)); //BITS(4, 0)
  INSTPAT("000000? ????? ????? 101 ????? 00110 11", srliw  , I, R(rd) = SEXT(BITS(src1, 31, 0) >> BITS(imm, 5, 0), 32));
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw  , I, R(rd) = SEXT((sword_t)src1 >> BITS(imm, 5, 0), 32));
  // ---------------------------------------------------------------------------------------------------------
  // ---------------------------------------------------------------------------------------------------------
