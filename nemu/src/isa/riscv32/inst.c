...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = U64(S32(src1) * S32(src2)) >> 32);
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = U64(S32(src1) * U32(src2)) >> 32);
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = U64(U32(src1) * U32(src2)) >> 32);
  ...
  INSTPAT("??????? ????? ????? 010 ????? 1110011", csrrs   , I, {R(rd) = CSR(imm); CSR(imm) |= CSR(imm);});
  // ---------------------------------------------------------------------------------------------------------
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(0xb, s->pc)); // Environment call from M-mode
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , R, {s->dnpc = cpu.csr[mepc]; cpu.csr[mstatus] &= (~0x1800);});
