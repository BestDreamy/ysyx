...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs   , I, {R(rd) = CSR(imm); CSR(imm) |= CSR(imm);});
  // ---------------------------------------------------------------------------------------------------------
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , I, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I, s->dnpc = isa_raise_intr(0xb, s->pc)); // Environment call from M-mode
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , I, s->dnpc = isa_ret_intr());
