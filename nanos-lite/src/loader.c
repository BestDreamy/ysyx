...
static uintptr_t loader(PCB *pcb, const char *filename) {
  ...
  for (int i = 0; i < elf_header.e_phnum; i++) {
    if (elf_segment_arr[i].p_type == PT_LOAD) {
      ramdisk_read((void *)elf_segment_arr[i].p_vaddr, elf_segment_arr[i].p_offset, elf_segment_arr[i].p_memsz);
      memset((void *)(elf_segment_arr[i].p_vaddr + elf_segment_arr[i].p_filesz), 0, elf_segment_arr[i].p_memsz - elf_segment_arr[i].p_filesz);
    }
  }
